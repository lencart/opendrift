...
class TestDelft3D(unittest.TestCase):
    ...
    def test_projected(self):
        ...
        try:
            ...
            assert isinstance(err, expected), (
                f"This raises a " f"{type(err)} error instead of {expected}"
            )
            print(f"Got {err}")
    ...
    def test_get_cube(self, case_choice=None):
        ...
        cube_slices = {
            ...
            "1D-HX": (slice(2, 3), slice(xx.min(), xx.max() + 1)),
            "1D-HY": (
        ...
        for acase in choices:
            ...
            assert cube.shape == raw.shape, f"Shapes fail for case {acase}"
    ...
    def test_interpolate_profiles(self):
        ...
        for case_name, cube_slice in cube_slices.items():
            ...
            zlv, zsg = r._get_depth_coords(slice_3d, self.zs)
            if case_name == "2D-V":
                # 2-DV
            ...
            for i, profile in enumerate(outs[case_name]):
                ...
                assert (
                    np.squeeze(in_result[case_name][aslice]).shape
                    == profile.shape
                ), f"Shapes fail for case {case_name}"
