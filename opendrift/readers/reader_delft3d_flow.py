...
def nearest(array, value, n=1):
    ...
    assert value.ndim == 1, (
        f"value has to be a single dimension vector"
...
class Reader(BaseReader, StructuredReader):
    ...
    def __init__(
        ...
        self._open_datastream(
            filename=filename, name=name, ensemble_member=ensemble_member
        )
    ...
    def __repr__(self):
        ...
        if hasattr(self, "z"):
            ...
            with np.printoptions(suppress=True, threshold=20) as opts:
                outStr += f"Vertical levels [sigma]: \n" f" {self.sigma}\n"
                outStr += f"Interpolated into zeta [m]:\n" f" {self.zlevels}\n"
        else:
            outStr += "Vertical levels [m]: \n  Not specified\n"
    ...
    def _parse_proj4(self):
        raise NotImplementedError(
            f"Unprojected coordinates not implemented"
    ...
    def _get_independent_vars(self):
        ...
        self.times = (
            self.Dataset[varname].data.astype("M8[ms]").astype("O").tolist()
        )
        ...
        try:
            ...
            raise NotImplementedError(
                (
                    "Delft3d-Flow reader only implemented"
    ...
    def _get_var_coords(ds, var):
        ...
        for i, dim in enumerate(ds[var].dims):
            ...
            dimcoords = list(
                filter(lambda x: dim in ds[x].dims, ds.coords.keys())
            )
            try:
                ...
                coordsout[i] = list(
                    filter(
                        lambda x: axis in ds.coords[x].long_name.lower(),
                        dimcoords,
                    )
                )[0]
        return coordsout
    ...
    def _overload_xy_grid(self):
        if isinstance(self.proj, fakeproj):
            ...
            if (
                ...
                and self.interpolator_filename is not None
            ):
                interpolator_filename = Path(
            ...
            if self.save_interpolator and Path(interpolator_filename).is_file():
                ...
                logger.info(
                    ("Making interpolator for lon,lat to x,y" " conversion...")
                )
                block_x, block_y = np.mgrid[
    ...
    def xy2lonlat(self, x, y):
        ...
        if self.projected:
            ...
            lon = map_coordinates(
                lon_in, [y, x], order=1, cval=np.nan, mode="nearest"
            )
            lat = map_coordinates(
                lat_in, [y, x], order=1, cval=np.nan, mode="nearest"
            )
            return (lon, lat)
    ...
    def _get_depth_coords(self, cube_slice, z_targets):
        ...
        dep = self._get_cube(
            "sea_floor_depth_below_sea_level", cube_slice[-2:]
        )[0]
        zeta = dep + eta
    ...
    def _get_xy(self, x, y):
        ...
        if (
            ...
            indx = np.arange(
                np.max([0, indx.min()]), np.min([indx.max() + 1, self.numx])
            )
        return indx, indy
    ...
    def _get_cube(self, varname, cube, testing=False):
        ...
        exp_slices = {3: (data.shape[0], 1, 1), 4: (1, data.shape[1], 1, 1)}
        ...
        try:
            ...
            for var, d3dvar in self._to_destagger.items():
                ...
                uv_dict[var] = self._destagger(
                    d3dvar, cube, hshape, testing=testing
                )
            # Rotate
    ...
    def _destagger(self, d3d_varname, cube, hshape, testing=False):
        ...
        data = 0.5 * (data[tuple(diff_slices[0])] + data[tuple(diff_slices[1])])
    ...
    def get_variables(
        ...
        xs, ys = np.meshgrid(variables["x"].flatten(), variables["y"].flatten())
        base_slice = (
